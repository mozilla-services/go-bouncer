...
func TestIsUserAgentOnlyCompatibleWithESR115(t *testing.T) {
	uas := []struct {
		...
		{"Mozilla/5.0 (Windows NT 6.1; WOW64; Trident/7.0; rv:11.0) like Gecko", true},                                                                 // IE 32bits Win7
...
func TestBouncerHandlerForWindowsOnlyCompatibleWithESR115(t *testing.T) {
	for _, tc := range []struct {
		...
		// This if for other firefox 32-bit products.
		for _, url := range []string{
		...
		// This if for other firefox 64-bit products.
		for _, url := range []string{
